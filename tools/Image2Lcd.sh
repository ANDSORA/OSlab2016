...
BPP=$(myread 0x1c 2 '"%d"' $1)
if [[ $BPP != 8 ]]			# modify this condition to check whether bbp has value 8. If not, print a message and exit
then
...
xxd -c 4 -s $palette_offset -l $palette_size -i $1
...
xxd -c $pad_width -s $pixel_offset -ps $1 | tac | grep -E -o "^.{$((2*$width))}" | xxd -r -ps | xxd -c $width -i
